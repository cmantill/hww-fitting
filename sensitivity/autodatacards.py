...
"""
Generating datacards according to SR categories
"""
...
class autodatacards:
    def __init__(self, args):
        ...
        self.outdir    = args.outdir + '/' + args.tag
        self.datadir   = args.idir
        self.chan      = args.chan
        self.verbose   = args.verbose

        from refs import datacardrefs
        self.refs = datacardrefs(self.year, self.chan, self.datadir, self.verbose)
        
        #enabling/disabling automcstats:
        self.usemcbinuncs = True
        
        #for generating histograms or not
        self.remakehists = ~args.notremakehists
    ...
    def cardsetup(self, startbinnum):        
        ...
        hm = histmaker(self.year, self.chan, self.shaperoot, self.refs, self.verbose)
    ...
    def makecard(self, cardname, binlabel, nbin, header, shapeline, yields):
        ...
        if self.verbose:
            print('TOTAL Background: ', round(sum(rates[1:])))
...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description='python autodatacards.py')
    ...
    parser.add_argument("--notremakehists",    dest="notremakehists", action='store_true', default=False,  help="do not regenerate shape histograms")
